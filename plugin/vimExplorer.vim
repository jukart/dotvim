...
let VEConf.filePanelHotkey.gotoPackage     = '<c-p>'
"single file actions
...
function! s:VEFilePanel.createActions()
    ...
    exec "nnoremap <silent> <buffer> " . g:VEConf.filePanelHotkey.gotoPackage .    " :call VE_OpenPythonPackage()<cr>"
...
"Set the python sandbox path
if exists("g:VEConf_pythonPackage")
    let VEConf.pythonPackage = g:VEConf_pythonPackage
else
    let VEConf.pythonPackage = g:VEPlatform.getHome()
endif

"Used in <c-p>
function! VE_OpenPythonPackage()
    echohl Special
    let workPath = input("Change package to (packagename): ",g:VEConf.pythonPackage,"file")
    echohl None
    if workPath == ''
        return
    endif
...
function! VE_OpenPath()
    echohl Special
    let workPath = input("Change path to (directory): ",g:VEPlatform.getcwd(),"file")
    echohl None
    if workPath == ''
        return
    endif
...
browseHistory           b               View browse history.
gotoPath                <c-g>           Change to another path.
openPythonPackage       <c-p>           Change to another python package.
